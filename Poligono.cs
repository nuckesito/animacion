...
namespace Proyecto_grafica
{
    public class Poligono
    {
       ...
       // public Matrix4 Matriz { get { return matriz; } set { matriz = value; } }
        ...
        public void Escalar(float e, Punto p)
        {
            Matrix4 M1 = Matrix4.CreateTranslation(-p.x, -p.y, -p.z);
            Matrix4 M2 = Matrix4.CreateScale(e);
            Matrix4 M3 = Matrix4.CreateTranslation(p.x, p.y, p.z);
            Matrix4 M = M1 * M2 * M3;
            matriz = M* matriz;
        }
        public void Rotar(float angulo, float x, float y, float z, Punto p)
        {
            ...
            if (x == 1 && y == 0 && z == 0)
                ...
                m = Matrix4.Identity;
            matriz=Matrix4.CreateTranslation(-p.x, -p.y, -p.z) * m * Matrix4.CreateTranslation(p.x, p.y, p.z)*matriz;
        }
